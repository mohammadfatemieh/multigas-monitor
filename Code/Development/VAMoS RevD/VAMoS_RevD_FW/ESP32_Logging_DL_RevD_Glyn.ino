 ...
 * 
 * Last tested: 12-Sept-2019 with VAMoS RevD
 */
...
#define PUMP 26
#define ALPHS_EN 13
//#define WARMUP_TIME 1800000 // 30 mins to warm up sensors (could be reduced to 25 mins)
#define WARMUP_TIME 5000 // 5 seconds to warm up sensors (should be 25 mins)
#define ACTIVE_TIME 60000 // 30 seconds
//#define ACTIVE_TIME 300000 // 5 mins
...
void setup() {
  ...
  pinMode(ALPHS1,INPUT);
  pinMode(ALPHS2,INPUT);
  pinMode(ALPHS3,INPUT);
  pinMode(ALPHS4,INPUT);
  pinMode(ALPHS5,INPUT);
  pinMode(ALPHS6,INPUT);
  pinMode(ALPHS7,INPUT);
  pinMode(ALPHS8,INPUT);
...
void logSDCard() {
  ...
  + String(DS18B20_temp) + "," + String(bme_t) + "," + String(hih_t) + "," + String(bme_rh) + "," + String(hih_rh) + "," + String(bme_p) + "," + String(ALPHS1_ADC) + "," + String(ALPHS2_ADC) + "," + String(ALPHS3_ADC) + "," + String(ALPHS4_ADC) + ","
  + String(ALPHS5_ADC) + "," + String(ALPHS6_ADC) + "," + String(ALPHS7_ADC) + "," + String(ALPHS8_ADC) + "\r\n";
...
void readAlphasense(){
  ...
  ALPHS1_ADC = analogRead(ALPHS1);
  delay(50);
  ALPHS2_ADC = analogRead(ALPHS2);
  delay(50);
  ALPHS3_ADC = analogRead(ALPHS3);
  delay(50);
  ALPHS4_ADC = analogRead(ALPHS4);
  delay(50);
  ALPHS5_ADC = analogRead(ALPHS5);
  delay(50);
  ALPHS6_ADC = analogRead(ALPHS6);
  delay(50);
  ALPHS7_ADC = analogRead(ALPHS7);
  delay(50);
  ALPHS8_ADC = analogRead(ALPHS8);
  delay(50);
  
  // Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 3.3V):
  ALPHS1_volts = ALPHS1_ADC * (3.3 / 4095.0);
  ALPHS2_volts = ALPHS2_ADC * (3.3 / 4095.0);
  ALPHS3_volts = ALPHS3_ADC * (3.3 / 4095.0);
  ALPHS4_volts = ALPHS4_ADC * (3.3 / 4095.0);
  ALPHS5_volts = ALPHS5_ADC * (3.3 / 4095.0);
  ALPHS6_volts = ALPHS6_ADC * (3.3 / 4095.0);
  ALPHS7_volts = ALPHS7_ADC * (3.3 / 4095.0);
  ALPHS8_volts = ALPHS8_ADC * (3.3 / 4095.0);
  ...
  Serial.print("Sensor 5 CO2 is: ");
  Serial.print(ALPHS5_volts);
  Serial.println(" V");
  
  Serial.print("Sensor 6 CO2 is: ");
  Serial.println(ALPHS6_volts);
  
  Serial.print("Sensor 7 (NC) is: ");
  Serial.println(ALPHS7_volts);  

  Serial.print("Sensor 8 (NC) is: ");
...
void runSensor()
{
    ...
    if (!file)
    {
        ...
        writeFile(SD, "/VAMoS_EnvData.txt", "Reading ID, Date, Time [UTC], DS18B20 [C], BME280_T [C], HIH_T [C], BME280_RH[%], HIH_RH [%], BME280_P [hPa], s1 [ADC], s2 [ADC], s3 [ADC], s4 [ADC], s5 [ADC], s6 [ADC] \r\n");
    }
    else
